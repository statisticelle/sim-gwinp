...
simScenarios <- function(scenarios, out, seed){
  ...
  for (s in 1:n_scen){
    ...
    # Combine all responses, assign unique cluster labels for modelling
    X <- rbind(X0, X1) %>% 
      arrange( B, Arm, C, pid) %>%
      group_by(B, Arm, C) %>%
      mutate(cid=cur_group_id()) 
    
    # Number of subjects within each replicate
    N <- X %>%
      group_by(B) %>%
      summarize(N0 = sum(ifelse(Arm==0, 1, 0)),
                N1 = sum(ifelse(Arm==1, 1, 0)))
    
    # Construct win fractions
    Y <- X %>%
      left_join(N, by=c('B'='B')) %>%
      group_by(B) %>%
      mutate(R1=rank(X1), R2=rank(X2)) %>%
      group_by(B, Arm) %>%
      mutate(G1=rank(X1), G2=rank(X2)) %>%
      group_by(B) %>%
    ...
    for (b in 1:scenario$B){
      ...
      # Fit linear mixed model to global win fractions
      # Using optim for more robust optimization
      mod <- try(
        lme(YG~Arm, random=~1|cid, data=Yb, 
      ...
      if (!inherits(mod, "try-error")){ # If no convergence failure...
        ...
        # Construct global win probability estimates
        est <- (mod_fest[2]+1)/2    # gwinp = (beta1+1)/2
        se  <- sqrt(mod_fvar[2,2])  # se(gwinp) = se(beta1)
